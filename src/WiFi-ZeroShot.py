...
def load_data(path_to_data, motion_sel, number_of_text):
    global T_MAX
    sixth_motion_count = 0
    ...
    for data_root, data_dirs, data_files in os.walk(path_to_data):
        for data_file_name in data_files:
            ...
            if label_1 == motion_sel[5]:
                if sixth_motion_count < number_of_text:
                    train_data.append(data_normed_1.tolist())
                    train_labels.append(label_1)
                    sixth_motion_count += 1
                else:
...
# 设定是否使用已有模型
use_pretrained_model = True  # True 代表加载已有模型，False 代表训练新模型
# 加载 WiFi 数据
...
# label:1-6
train_data, test_data, train_labels, test_labels = load_data(data_dir, ALL_MOTION, number_for_test)
print("data finish")
